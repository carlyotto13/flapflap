...
from old_code_backup.settings import (
    W,
    H,
    obstacle_size,
    obstacle_gap,
...
class World:
    ...
    def _add_obstacle(self):
        obstacle_pair_size = random.choice(obstacle_pair_sizes)
        top_obstacle_height, bottom_obstacle_height = (
            obstacle_pair_size[0] * obstacle_size,
            obstacle_pair_size[1] * obstacle_size,
        )
        obstacle_top = Obstacle(
            (W, 0 - (bottom_obstacle_height + obstacle_gap)), obstacle_size, H, True
        )
        obstacle_bottom = Obstacle(
            (W, top_obstacle_height + obstacle_gap), obstacle_size, H, False
        )
        self.obstacles.add(obstacle_top)
        self.obstacles.add(obstacle_bottom)
        self.current_obstacle = obstacle_top
    ...
    def _handle_collisions(self):
        ...
        # for collision checking
        if (
            pygame.sprite.groupcollide(self.player, self.obstacles, False, False)
            or animal.rect.bottom >= H
            or animal.rect.top <= 0
        ):
            self.playing = False
            self.game_over = True
        else:
